...
const updateWeatherOutput = async (json, site) => {
    ...
    const asidetext = document.querySelector('#inputtext');
    asidetext.textContent = `${site} - LAT: ${latitude} LON: ${longitude}`
    ...
    if(rowCount > 5)
    {
        ...
        var metarButtonCell = row.insertCell(4)
        metarButtonCell.innerHTML = `<button onclick="refreshMETAR('${station_id}')"> Reload Station </button>`
